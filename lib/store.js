...
export const useStore = create((set, get) => ({
  ...
  updateOtpItem: async (id, updates) => {
    ...
    try {
      ...
      const response = await fetch(`/api/otp/${encodeURIComponent(id)}/`, {
        method: "PATCH",
        headers: {
      ...
      // Se o id (alias) foi alterado, remover o antigo e adicionar o novo
      const newId = updates.newId || id;
      
      set((state) => {
        if (updates.newId) {
          // Se houve mudança de ID, remover o antigo e adicionar o novo
          return {
            otpItems: state.otpItems.filter(item => item.id !== id)
              .concat([{ ...updatedItem, id: newId }]),
            isLoading: false
          };
        } else {
          // Atualização normal sem mudar o ID
          return {
  ...
  removeOtpItem: async (id) => {
    ...
    try {
      const authHeaders = get().getAuthHeaders()
      console.log(`Store - Removendo item ${id}, token:`, 
      ...
      if (!response.ok) {
        const errorData = await response.json()
        console.error(`Store - Erro ao remover item ${id}:`, errorData)
