...
// Helper function to get all OTP items from the 'otp-items' node
async function getAllOtpItems() {
  const db = admin.database();
  ...
  try {
    ...
    return Object.entries(otpData).map(([id, data]) => ({
      id, // The unique key of the item
      ...data, // All other data associated with the item
    }));
  } catch (error) {
    console.error("Error getting all OTP items:", error);
    return [];
  }
}

// GET - Fetch all OTP items from 'otp-items'
export async function GET(request) {
  ...
  if (!decoded.userId) {
    console.log("API OTP - Token sem userId")
    return NextResponse.json(
  ...
  try {
    const allOtps = await getAllOtpItems();
    console.log("API OTP - Dados recuperados com sucesso, quantidade:", allOtps.length);

    // Retornar os dados no formato esperado pelo frontend: { items: [...] }
    return NextResponse.json({ items: allOtps });
  } catch (error) {
